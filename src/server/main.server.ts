...
import { iceEndpoint } from "./ice.js";
...
const websocketServer = Bun.serve<WebsocketData>({
	...
	async fetch(request, server) {
		...
		if (url.pathname === "/api/relay") return handleUpgradeWebsocket(server, request);
		if (url.pathname === "/api/ice") return iceEndpoint(server, request);
...
console.log(`WebSocket server listening on http://${websocketServer.hostname}:${websocketServer.port}`);

function ok(text: string) { return new Response(text); }
function bad(text: string) { return new Response(text, { status: 400 }); }

function handleUpgradeWebsocket(server: Bun.Server<WebsocketData>, request: Request) {
	// Get room ID
	const url = new URL(request.url);
